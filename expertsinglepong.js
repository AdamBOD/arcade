(function() {
    ...
    function init() {
        canvas = document.getElementById("expertsinglepong");
    ...
    function draw () { //Draw is the main function to draw all the elements on the canvas; the ball, the bricks, the platform, the score and the lives *** STILL NEED TO IMPLEMENT SCORE AND LIVES ***
        ...
        if ( x + radius < 0 ) {
            ...
            if (rightlives == 1) {
                ...
                x = 500;
                y = 375;
                xChange = 1; //In all the other difficulties I inverse the xChange and yChange but because of the small range of the AI paddle, instead of modifying the yChange I left the direction going to the player
                yChange = 1;
                platform1X = 20, platform1Y = 300;
                platform2X = 970, platform2Y = 300;
            }
        ...
        if (x + radius > platform1X && x - radius < platform1X && (y > platform1Y && y < platform1Y + platformHeight)){
            xChange = -xChange;
            xChange = xChange * 1.075;
            yChange = yChange * 1.075;
        }
    ...
    function stop() {
     ...
     for (i = 0; i < 2; i ++) {
        var url = 'expertpongscore.py?username=' +username+ '&score=' + score;
